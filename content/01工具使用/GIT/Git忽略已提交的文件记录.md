...
很多时候某些本地生成的文件如[编译器](../IDE/编译器.md)生成的文件，不想让 [git](Git.md) 跟踪变更并提交 有两种情况：
...
**注意**：**git rm -r --cached** 是高危操作，在命令后一定要指定目录或文件 ，否则会删除所有缓存。

git rm -r --cached tools/hmi_font/out/report.json

git rm -r --cached config.json

git rm -r --cached base_A7/libcextern/Release
git rm -r --cached base_A7/libpcap/Release
git rm -r --cached base_A7/libsunrpc/Release
git rm -r --cached base_A7/libVxWorks/Release
git rm -r --cached base_A7/openssl/Release
git rm -r --cached base_A7/tcpdump/Release
git rm -r --cached base_A7/unfsd/Release
