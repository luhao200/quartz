...
- **switch 表达式的类型：** switch 语句中的表达式必须是整数类型（char、short、int或[枚举](枚举.md)），或者是能够隐式转换为整数类型的表达式。
- **case 标签和表达式的范围：** switch 语句的 case 标签可以是整数常量表达式，但不能是浮点数或[字符串](字符串.md)。
...
- **break 语句的使用：** 在每个 case 标签的代码块结束处通常需要使用 break 语句来终止 switch 语句的执行。如果没有 break 语句，**程序将会继续执行下一个 case 标签中的代码（ #import/pf 不会去判断，直接执行），直到遇到 break 语句或 switch 语句结束。**
