...
在 C 语言中，命令行参数是一种从命令行获取输入的方法，可以用于运行程序时传递信息给程序。命令行参数通过 main [函数](函数.md)的参数传递给程序。main [函数](函数.md)的原型可以是如下两种形式之一：
...
- **`argv` (argument vector)**: 是一个指向[字符串](字符串.md)[数组](数组.md)的[指针](指针.md)，其中每个[字符串](字符串.md)是一个命令行参数。[数组](数组.md)的第一个元素（即 `argv[0]`）通常是程序的名称。接下来的元素是传递给程序的命令行参数。
...
应当指出的是，`argv[0]` 存储程序的名称，`argv[1]`是一个指向第一个命令行参数的[指针](指针.md)，`*argv[n]` 是最后一个参数。如果没有提供任何参数，`argc` 将为 1，否则，如果传递了一个参数，`argc` 将被设置为 2。
...
- 命令行参数通常是[字符串](字符串.md)，如果需要将其转换为数值类型，可以使用标准库[函数](函数.md)如 `atoi` 或 `strtol`。
