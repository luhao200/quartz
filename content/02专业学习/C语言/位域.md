...
上面的结构定义指示 C [编译器](../../01工具使用/IDE/编译器.md)，age 变量将只使用 3 位来存储这个值，如果您试图使用超过 3 位，则无法完成。
...
以上代码定义了一个名为 **struct bs** 的[结构体](结构体.md)，data 为 bs 的[结构体](结构体.md)变量，共占四个字节：

对于位域来说，它们的**宽度不能超过其[数据类型](数据类型.md)的大小**，在这种情况下，int 类型的大小通常是 4 个字节（32位）。
